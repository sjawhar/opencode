...
describe("opencode run (non-interactive subprocess)", () => {
  ...
  // The budget covers a full CLI cold start on a contended CI runner (measured
  // 15.4s against the previous 15s bound); the regression this guards is an
  // indefinite hang, so the wide margin does not weaken the test.
  cliIt.concurrent(

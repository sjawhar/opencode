...
describe("Npm.sanitize", () => {
  ...
  test("handles git https specs", () => {
    ...
    const expected = "acme@git+https_//github.com/opencode/acme.git"
...
describe("Npm.install", () => {
  test("respects omit from project .npmrc", async () => {
    ...
    const installed = await fs.readdir(path.join(tmp.path, "node_modules"))
    expect(installed).toContain("prod-pkg")
    expect(installed).not.toContain("dev-pkg")
  })
})
